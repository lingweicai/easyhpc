...
export default function SrunLauncher() {
    ...
    const fetchDirectory = async (path: string): Promise<MyTreeViewDataItem[]> => {
        try {
            ...
            console.error(_("Error reading directory:"), err);
            return [];
        }
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            console.log(_("srun result:"), result);
            setSrunResults(result);
            refresh();
        } catch (err) {
            console.error(_("srun error:"), err);
            alert(_("Failed to launch job:") + ` ${err.message || err}`);
    ...
    return (
        <>
            ...
            >
                <ModalBody>
                    <Form isHorizontal>
                        <FormGroup label={_("Application file (.sh or executable)")} isRequired fieldId="app-path">
                            <InputGroup>
                                <TextInput
                                    id="app-path"
                                    value={appPath}
                                ...
                                <InputGroupItem isPlain>
                                    <Tooltip content={_("Browse Files")}>
                                        <Button
                                            variant="control"
                                            icon={<FolderOpenIcon />}
                        ...
                        <FormGroup label={_("Number of nodes")} isRequired fieldId="num-nodes">
                            <TextInput
                                id="num-nodes"
                                type="number"
                                value={numNodes}
                        ...
                        <FormGroup label={_("Number of processes")} isRequired fieldId="num-procs">
                            <TextInput
                                id="num-procs"
                                type="number"
                                value={numProcs}
